import nodeIndex from './nodes/nodeIndex';
...
export function buildNodeTree(ast, tokens) {
    ast.range = tokens.length > 0 ?
        [tokens[0].range[0], tokens[tokens.length - 1].range[1]] : [0, 0];
    return buildNodeTreeItem(ast, tokens, 0, tokens.length);
}

function buildNodeTreeItem(ast, tokens, start, end) {
    ...
    while (index < end) {
        ...
        if (childElement && token.range[0] >= childElement.range[0]) {
            ...
            if (childElement.range[1] === token.range[1]) {
                ...
                children.push(buildNodeTreeItem(childElement, tokens, elementStartTokenIndex, index + 1));
                childElementIndex++;
    ...
    let TokenClass = nodeIndex[ast.type];
