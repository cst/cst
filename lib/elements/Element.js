...
export default class Element {
    ...
    replaceChild(child, firstRefChild, lastRefChild) {
        // TODO: Implement
    }

    insertChildBefore(child, referenceChild) {
        if (child === referenceChild) {
            throw new Error('New element cannot equal reference element.')
        }
...
function fillWithArray(inputArray, array, offset) {
    for (let i = 0; i < inputArray.length; i++) {
        array[offset + i] = inputArray[i];
    }
}
