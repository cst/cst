    ...
    while (index < end) {
        ...
        if (childElement && token.range[0] >= childElement.range[0]) {
            ...
            if (childElement.range[1] === token.range[1]) {
                ...
                children[children.length] = buildElementTreeItem(
                    childElement, tokens, elementStartTokenIndex, index + 1
                );
                childElement = childElements[++childElementIndex];
            }
        } else {
            children[children.length] = new Token(token.type, token.value);
        }
        index++;
    }
